...
| 14 | F15-T14 | Framing survives confirm, too | T13 fixed the editing/exporting states but left the ones after them: `ImagePreviewConfirmed` and `ImagePreviewCreatingSession` still dropped **both** `quarterTurns` and `cropRect`, and the preview is visible behind the quality sheet and the session veil — so it flipped back to the raw, unrotated photo exactly while the user was being asked to judge it. All four post-confirm states now carry the framing (the two terminal ones because the builder still renders one frame before the router swaps the screen out); cubit + screen regression tests | DONE |
