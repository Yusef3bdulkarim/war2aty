...
| 16 | F13-T16 | No-silent-fallback guarantee | Once online is chosen, any failure maps to retry-only, never invokes `OcrEngine`/Tesseract; tests assert Tesseract never constructed on a failed-online run | DONE |
| 17 | F13-T17 | `rawValue` wire-through + confidence-parity test | `rawValue` added to date/amount entities; new phone/reference entities; `verificationStatus` proven absent from every Flutter-facing type | DONE |
| 18 | F13-T18 | Privacy copy + CLAUDE.md update | `privacyPointTextOnly` and siblings reworded, no provider names; `CLAUDE.md` §7/Project Context updated; README totals/critical-path updated | DONE |
| 19 | F13-T19 | End-to-end verification + kill-switch flip | Real Azure + Google sandbox run through the full pipeline incl. a Google-second-opinion case and a `needsUserReview: true` case; Azure delete verified via follow-up GET; only then `azureOcrEnabled` flipped on | DONE |

**T19 verification (2026-08-11):** Ran the real online pipeline locally (`supabase functions serve` against the local stack, `azure_ocr_enabled` set `true` in the **local** `app_runtime_config` only) with a synthetic test bill image sent through the actual image-intake `analyze-document` route, real Azure Document Intelligence (`war2aty-ocr-sandbox.cognitiveservices.azure.com`, `prebuilt-read`) and real Google Document AI (`war2aty-ocr-sandbox` GCP project, `OCR_PROCESSOR` `5c1049eb9e679e70`).
- **Google-second-opinion case:** an intentionally ambiguous numeric date (`03/04/2026`, both parts ≤12) left Azure's own verdict non-`verified`; confirmed via temporary instrumentation (reverted after, `git diff` clean) that `needsGoogleSecondOpinion` fired and the Google call succeeded and corroborated it (`confidence:"high"`, `needsUserReview:false` on the wire).
- **`needsUserReview: true` case:** a second image with a heavily degraded due-date field produced no date candidate at all (`status:"partial"`, `missing_fields:["Due Date"]`) — since no critical field triggered Google that run, Azure's own unverified `references[]` fell through the "Google not consulted" branch and came back `needsUserReview:true` on the wire, genuinely.
- **Azure delete verified via follow-up GET:** captured the real `Operation-Location` URL via temporary logging (reverted after), and a manual follow-up `GET` after the request completed returned `404 {"code":"NotFound","message":"Analyze result does not exist."}` — the delete-in-`finally` (T04) is confirmed working against the live service, not just asserted by a mock in unit tests.
- **Remote deployment + flip (2026-08-11):** the user's Supabase project `war2aty` (`jecujrsvbmashkpobtsz`) was empty (no migrations/secrets/functions). Pushed all 6 migrations, pushed all 14 Edge Function secrets (Groq + Azure + Google), pushed auth config (anonymous sign-ins on, matching `config.toml`), and deployed `analyze-document`/`get-usage`/`health`. Verified remotely with a real text-shape call (200, Groq-classified) and, after explicit user confirmation, flipped `azure_ocr_enabled=true` and verified with a real image-shape call — full parity with the local runs above (200, Azure+Google+Groq, same result shape). The online pipeline is now live on this project.
